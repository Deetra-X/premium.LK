// Fix the SalesManagement component's rendering issue
// and create a clean AccountOrders.ts file

import * as fs from 'fs';
import * as path from 'path';

// Fix for SalesManagement.tsx
const salesManagementPath = path.join(__dirname, 'src', 'components', 'SalesManagement.tsx');
let salesManagementContent = fs.readFileSync(salesManagementPath, 'utf-8');

// Fix the status.charAt error by adding a null check
salesManagementContent = salesManagementContent.replace(
  /sale\.status\.charAt\(0\)\.toUpperCase\(\) \+ sale\.status\.slice\(1\)/g,
  'sale.status ? (sale.status.charAt(0).toUpperCase() + sale.status.slice(1)) : "Pending"'
);

fs.writeFileSync(salesManagementPath, salesManagementContent, 'utf-8');
console.log('✅ Fixed SalesManagement.tsx');

// Create clean AccountOrders.ts
const accountOrdersContent = `import { v4 as uuidv4 } from 'uuid';
...
const accountOrdersPath = path.join(__dirname, 'src', 'api', 'AccountOrders.ts');
fs.writeFileSync(accountOrdersPath, accountOrdersContent, 'utf-8');
console.log('✅ Created clean AccountOrders.ts');
