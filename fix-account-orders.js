// This script will replace the problematic AccountOrders.ts file with a clean version
// Run with: node fix-account-orders.js

const fs = require('fs');
const path = require('path');

// Path to AccountOrders.ts
const accountOrdersPath = path.join(__dirname, 'src', 'api', 'AccountOrders.ts');

// Clean version of AccountOrders.ts
const cleanContent = `import { v4 as uuidv4 } from 'uuid';
...
fs.writeFileSync(accountOrdersPath, cleanContent, 'utf8');

console.log('✅ Successfully fixed AccountOrders.ts!');
console.log('The file now contains only client-side code and should work correctly.');
